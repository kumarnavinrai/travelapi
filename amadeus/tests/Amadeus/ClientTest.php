...
class ClientTest extends BaseTestCase
{
    ...
    public function testCanCreateClientWithAuthOptionsAndSessionParams()
    {
        $par = new Params([
            'authParams' => [
                'officeId' => '1ASIWOTANA4',
                'originatorTypeCode' => 'U',
                'userId' => 'WSNA4OTA',
                'passwordData' => base64_encode('uYTzW')
            ],
            'sessionHandlerParams' => [
                'wsdl' => $this->makePathToDummyWSDL(),
                'stateful' => true,
    ...
    public function testCanDoDummyPnrTransferOwnershipCall()
    {
        ...
        $mockSessionHandler
            ->expects($this->once())
            ->method('sendMessage')
            ->with('PNR_TransferOwnership', $expectedPnrResult, ['endSession' => false])
            ->will($this->returnValue($mockedSendResult));
        $mockSessionHandler
            ->expects($this->once())
            ->method('getMessagesAndVersions')
    ...
    public function testCanSendMiniRuleGetFromPricing()
    {
        ...
        $mockSessionHandler
            ->expects($this->once())
            ->method('sendMessage')
            ->with('MiniRule_GetFromPricing', $expectedMessageResult, ['endSession' => false])
            ->will($this->returnValue($mockedSendResult));
        $mockSessionHandler
            ->expects($this->never())
            ->method('getLastResponse');
    ...
    public function testCanSendSalesReportsDisplayQueryReport()
    {
        ...
        $mockSessionHandler
            ->expects($this->once())
            ->method('sendMessage')
            ->with('SalesReports_DisplayQueryReport', $expectedMessageResult, ['endSession' => false])
            ->will($this->returnValue($mockedSendResult));
        $mockSessionHandler
            ->expects($this->never())
            ->method('getLastResponse');
