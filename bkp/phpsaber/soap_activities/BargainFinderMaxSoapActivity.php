...
class BargainFinderMaxSoapActivity implements Activity {
    ...
    public function run(&$sharedContext) {
      ...
      if(isset($_REQUEST['origin']) && $_REQUEST['origin'] != "")
      {
        ...
        $xmlRequest = $this->getRequest($origin,$destination,$departuredate,$returndate,$limit,$outboundflightstops,$outbounddeparturewindow,$includedcarriers,$inboundstopduration,$passengercount,$lengthofstay);
    ...
    private function getRequest($origin,$destination,$departuredate,$returndate,$limit,$outboundflightstops,$outbounddeparturewindow,$includedcarriers,$inboundstopduration,$passengercount,$lengthofstay) {
      ...
      {  
        $onewayxml = '<OTA_AirLowFareSearchRQ Version="1.9.2"  xmlns="http://www.opentravel.org/OTA/2003/05">';
          ...
          $onewayxml .= '<TravelPreferences ValidInterlineTicket="true" >';
            $onewayxml .= '<CabinPref Cabin="Y" PreferLevel="Preferred" ></CabinPref>';
