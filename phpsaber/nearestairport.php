...
include_once 'soap_activities/SabreCommandLLSRQSoapActivity.php';
include_once 'soap_activities/BargainFinderAlternateAirportSoapActivity.php';
...
$mainarrayorresulresults['amadeusresult'] = array();
      ...
      if(isset($_REQUEST['origin']) && $_REQUEST['origin'] != "")
      {
        ...
        $workflow = new Workflow(new SabreCommandLLSRQSoapActivity());
        ...
        $arrayofres = (array) $result; 
          ...
          if($arrayofres)
          {
               ...
               // echo "<pre>"; print_r($bypassfirstelementofarray); die; 

                $requestofxml= $bypassfirstelementofarray['SabreCommandLLSRS'];
                
                $arraybfmrequestarray = XML2Array::createArray($requestofxml);
                //echo "<pre>"; print_r($arraybfmrequestarray); die;
                                
                $arraybfmrequestarray = reset($arraybfmrequestarray);  //[SOAP-ENV:Envelope]
 
                $arraybfmrequestarray = $arraybfmrequestarray['soap-env:Body'];
               
                $arraybfmrequestarray = reset($arraybfmrequestarray); // [OTA_AirLowFareSearchRS]
                //echo "<pre>"; print_r($arraybfmrequestarray); die;
                if(isset($arraybfmrequestarray['Response']))
                {  

                  $arraybfmrequestarray = $arraybfmrequestarray['Response']; 
                  
                  $ans = preg_split("/\r\n|\n|\r/", $arraybfmrequestarray);

                  if(trim($ans[0]) == 'NEAREST AIR SERVICE FROM')
                  {

                    //echo "<pre>"; print_r($ans); die; 
                    $nearairposr[] = getAirpoCode($ans[4]);
                    $nearairposr[] = getAirpoCode($ans[5]);
                    $nearairposr[] = getAirpoCode($ans[6]);
                    $nearairposr[] = getAirpoCode($ans[7]);
                    $nearairposr[] = getAirpoCode($ans[8]);
                    $nearairposr[] = getAirpoCode($ans[9]);
                    $nearairposr[] = getAirpoCode($ans[10]);
                    ...
                    {
                      $workflow = new Workflow(new BargainFinderAlternateAirportSoapActivity($nearairposr));
                      $resultbfm = $workflow->runWorkflow();
                      //echo "<pre>"; print_r($resultbfm); die;
                      $arrayofresbfm = (array) $resultbfm; 

                      $ansfromfunction = arrangeDataOfBfmSoap($arrayofresbfm);
                      $ansfromfunction = isset($ansfromfunction['amadeusresult'])&&$ansfromfunction['amadeusresult']?$ansfromfunction['amadeusresult']:array();

                      $mainarrayorresulresults['amadeusresult'] = array_merge($mainarrayorresulresults['amadeusresult'], $ansfromfunction);


                      $workflow = new Workflow(new BargainFinderAlternateAirportTwoSoapActivity($nearairposr));
                      $resultbfm = $workflow->runWorkflow();
                      //echo "<pre>"; print_r($resultbfm); die;
                      $arrayofresbfm = (array) $resultbfm; 
                      ...
                      $workflow = new Workflow(new BargainFinderAlternateAirportThreeSoapActivity($nearairposr));
                      $resultbfm = $workflow->runWorkflow();
                      //echo "<pre>"; print_r($resultbfm); die;
                      $arrayofresbfm = (array) $resultbfm; 
                ...
                }else{ $arrayofresbfm = array(); }
              }else{
                $arrayofresbfm = array();
              }  


          } //if check origin brace  
...
die;
      ...
      //function starts
function arrangeDataOfBfmSoap($arrayofresbfm){
      include 'data/listofairport.php';
      include 'data/listwithcode.php';
      $passengercount = 1; 
      $mainarrayorresulresults['amadeusresult'] = array();   

      

      if(isset($arrayofresbfm) && $arrayofresbfm)
      {
...
$mainarrayorresulresults['amadeusresult'] = array_filter($mainarrayorresulresults['amadeusresult'],function($a) { 
 if(isset($a['indeparturedate'])){ return (( $a['outdeparturedate'] >=$_SESSION['filterdatelow'] && $a['outdeparturedate'] <= $_SESSION['filterdate']) || ($a['indeparturedate']>=$_SESSION['filterdatelow']&&$a['indeparturedate'] <= $_SESSION['filterdate'])); }else{
  return ($a['outdeparturedate']>=$_SESSION['filterdatelow']&&$a['outdeparturedate'] <= $_SESSION['filterdate']);
    ...
    return $mainarrayorresulresults;    
}// arrangeDataOfBfmSoap function ends here
